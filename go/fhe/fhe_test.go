...
if err != nil { t.Fatalf("KeyGen error: %v", err) }
...
cases := []struct{ name string; trials int }{
...
ct1, err := adp.Enc(pk, m1); if err != nil { t.Fatalf("enc1: %v", err) }
ct2, err := adp.Enc(pk, m2); if err != nil { t.Fatalf("enc2: %v", err) }


// Add test: Dec(Enc(m1)+Enc(m2)) == (m1+m2) mod modulus
addCT, err := adp.Add(ct1, ct2); if err != nil { t.Fatalf("add: %v", err) }
addPT, err := adp.Dec(sk, addCT); if err != nil { t.Fatalf("dec add: %v", err) }
...
mulCT, err := adp.Mul(ct1, ct2); if err != nil { t.Fatalf("mul: %v", err) }
mulPT, err := adp.Dec(sk, mulCT); if err != nil { t.Fatalf("dec mul: %v", err) }
