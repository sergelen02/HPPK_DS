...
// 샘플러들 (실험용)
func randMod(m *big.Int) *big.Int { x,_ := rand.Int(rand.Reader, m); if x.Sign()<0 {x.Add(x,m)}; return x }
func coprime(a,b *big.Int) bool { g := new(big.Int).GCD(nil,nil,a,b); return g.Cmp(big.NewInt(1))==0 }

// Placeholder: 공개다항 P(x),Q(x) 계수(길이 n)를 외부에서 공급하거나 여기서 생성
func makePQ(n int, p *big.Int) (P, Q []*big.Int) {
	P = make([]*big.Int, n); Q = make([]*big.Int, n)
	for i:=0;i<n;i++{
		P[i]=randMod(p); Q[i]=randMod(p)
	}
	return
}
...
func computeMuNu(pp *Params, P,Q []*big.Int) (mu,nu []*big.Int) {
	n := len(P); mu = make([]*big.Int, n); nu = make([]*big.Int, n)
	R := pp.R()
	for i:=0;i<n;i++{ mu[i]=new(big.Int).Rsh(R,2); nu[i]=new(big.Int).Rsh(R,3) } // TODO: 실제 값으로 교체
	return
}
...
func KeyGen(pp *Params, n int) (*SecretKey, *PublicKey) {
	...
	for { R1,S1 = randMod(p),randMod(p); if S1.Sign()==0{S1=big.NewInt(1)}; if coprime(R1,S1){break} }
	for { R2,S2 = randMod(p),randMod(p); if S2.Sign()==0{S2=big.NewInt(1)}; if coprime(R2,S2){break} }
	// 3) β
	beta := randMod(p)
